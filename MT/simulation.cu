...
int main(){
  ...
  // int SIMTYPE = 1;
   for(int S = 256; S <= 256; S<<=1){
    ...
    float* loc_burnDist;
    bool* check;
    gpuRoth = (int*)malloc(sim.simDimX*sim.simDimY*3*sizeof(int));
    ...
    err = cudaMalloc( (void**) &g_rothData, sim.simDimX*sim.simDimY*3*sizeof(int));
    ...
    err = cudaMemcpy(g_rothData, gpuRoth, sim.simDimX*sim.simDimY*3*sizeof(int), cudaMemcpyHostToDevice);
